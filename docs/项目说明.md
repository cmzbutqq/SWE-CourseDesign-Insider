...
```mermaid
flowchart LR
    User["浏览器用户"] --> Frontend["Frontend<br/>Vue 3 + Vite"]

    Frontend --> Backend["Backend<br/>Spring Boot"]
    Frontend --> Grafana["Grafana"]
    Frontend --> Prometheus["Prometheus"]
    Frontend --> SkyUI["SkyWalking UI"]

    AppAgent["monitor-agent@app-node"] --> Backend
    MidAgent["monitor-agent@middleware-node"] --> Backend

    Backend --> MySQL["MySQL<br/>平台元数据"]

    Backend --> Prometheus
    AppNode["app-node<br/>sample-service + node_exporter"] --> Prometheus
    MidNode["middleware-node<br/>middleware-service + nginx_exporter + redis_exporter + node_exporter"] --> Prometheus

    AppNode --> SkyOAP["SkyWalking OAP"]
    MidNode --> SkyOAP
    SkyOAP --> SkyUI
```
